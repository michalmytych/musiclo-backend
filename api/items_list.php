            ...
            case 'songs':
                $query = "SELECT 
                        ...
                        if (isset($_GET['phrase'])) {
                            $phrase = $_GET['phrase'];
                            $query = $query . " WHERE songs.name LIKE '%$phrase%'";
                        }
                        $query = $query . " GROUP BY songs.id LIMIT ".$page_start.", ".$limit.";";       
                break;         
            case 'albums':
                $query = "SELECT 
                        albums.id,
                        ...
                        if (isset($_GET['phrase'])) {
                            $phrase = $_GET['phrase'];
                            $query = $query . " WHERE albums.name LIKE '%$phrase%'";
                        }
                        $query = $query . " GROUP BY albums.id LIMIT ".$page_start.", ".$limit.";";
                break;
            case 'artists':
                $query = "SELECT 
                        artists.id, 
                        ...
                        if (isset($_GET['phrase'])) {
                            $phrase = $_GET['phrase'];
                            $query = $query . " WHERE artists.name LIKE '%$phrase%'";
                        }
